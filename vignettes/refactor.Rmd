...
```
\
In a real-world situation, it is likely that a lot of variables have to be decoded. For this task, we created `index_cfactor`. First, we need some sample data.
```{r}
...
sapply(final, class)
```
`index_cfactor` converts all columns in `data` that have a match in `index` to factors with the respective encoding using `cfactor`. Further arguments are `category`, `label` and `encoding` referring to the colunm names in index that contain the respective information.
