...
### detect levels
`cfactor` should detect sequences in strings if the argument `ordered` is set to `TRUE` order the levels. The default behavior of `factor` is first convert `x` to character, take the unique values and sort the characters (`sort(unique(as.character(x)))`), which is fine if the string ordering was the same as ordering the numbers within the string, but in general that is not the case.
...
However, in the general case, where number of digits might be destinct, this does not work anymore. The category "EUR 100 - 101" comes second, but it should be the last.
```{r}
hard_to_dectect <- c("EUR 21 - EUR 22", "EUR 100 - 101", "EUR 1 - EUR 10", "EUR 11 - EUR 20")
factor(hard_to_dectect, ordered = T) # factor messes up 
```

`cfactor` detects levels using regular expressions. Concretely, it extracts the substrings before the `sep`, removes all letters in `x` and orders the remaining numbers.
```{r}
